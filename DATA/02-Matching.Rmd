...
We can print the list of matches. Each number represents the row ID of the observation, and an NA in the control group means no match was found for that particular democratic district. 
...
```{r}
write.csv( balanced.data, "matched-pairs.csv", row.names=F )
```
...
write.csv( matches, "matched-pairs-index.csv", row.names=F )
```
