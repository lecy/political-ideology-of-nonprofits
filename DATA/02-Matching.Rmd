...
dat <- 
  dat %>% 
  select( tract.key, pop10, Shape_area, medianage, 
...
```{r}
vote.dem <- dat$Pres_D_08 / (dat$Pres_D_08 + dat$Pres_R_08)

dat$dem.super <- vote.dem > 0.70 

dat$repub.super <- vote.dem < 0.30 

match.dat <- dat[ vote.dem < 0.30 | vote.dem > 0.70 , ]
